...
data = pd.read_csv("/Users/macpro/Desktop/Roula/dataha/streamlit/data.csv")
...
# Display the pie chart
    ...
    with col1:
        ...
        conditions = [
            (data['AGE'] >= 0) & (data['AGE'] <= 17),
            (data['AGE'] >= 18) & (data['AGE'] <= 24),
            (data['AGE'] >= 25) & (data['AGE'] <= 29),
            (data['AGE'] >= 30) & (data['AGE'] <= 34),
            (data['AGE'] >= 35) & (data['AGE'] <= 39),
            (data['AGE'] >= 40) & (data['AGE'] <= 44),
            (data['AGE'] >= 45) & (data['AGE'] <= 49),
            (data['AGE'] >= 50) & (data['AGE'] <= 54),
            (data['AGE'] >= 55) & (data['AGE'] <= 59),
            (data['AGE'] >= 60) & (data['AGE'] <= 64),
            (data['AGE'] >= 65) & (data['AGE'] <= 69),
            (data['AGE'] >= 70) & (data['AGE'] <= 74),
            (data['AGE'] >= 75) & (data['AGE'] <= 79),
            (data['AGE'] >= 80)]
...
# Create a new column 'age_bracket_number' based on the conditions and age bracket numbers
    data['age_bracket_number'] = np.select(conditions, age_bracket_numbers, default=np.nan)

# Count the number of individuals in each age bracket
    age_counts = data['age_bracket_number'].value_counts().sort_index()
...
# Customize the bar chart layout
    ...
    class_counts = data['Class_treatment_M0'].value_counts().reset_index()
...
if menu_id == "Overview":
    ...
    HDRS_mean_male = [data[data['GENDER'] == 1]['HDRS_M0'].mean(),
                        data[data['GENDER'] == 1]['HDRS_M3'].mean(),
                        data[data['GENDER'] == 1]['HDRS_M6_bon'].mean()]

    # Calculate the median weight values for each time point for gender 2 (female)
    HDRS_mean_female = [data[data['GENDER'] == 2]['HDRS_M0'].mean(),
                        data[data['GENDER'] == 2]['HDRS_M3'].mean(),
                        data[data['GENDER'] == 2]['HDRS_M6_bon'].mean()]
    ...
    group_counts = data.groupby(['GENDER', 'drugfree_ka']).size().reset_index(name='Count')
    ...
    for variable in variables:
        values_m0.append(data[f'{variable}_M0'].mean())
        values_m3.append(data[f'{variable}_M3'].mean())
        values_m6.append(data[f'{variable}_M6'].mean())
...
if menu_id == "Analysis":
    ...
    male_not_mets = data[(data['GENDER'] == 1) & (data['mets_M0'] == 0)]['id'].count()
    # Filter the data for gender 1 (male) and METS status 1 (developed METS)
    male_mets = data[(data['GENDER'] == 1) & (data['mets_M0'] == 1)]['id'].count()

    # Filter the data for gender 2 (female) and METS status 0 (not developed METS)
    female_not_mets = data[(data['GENDER'] == 2) & (data['mets_M0'] == 0)]['id'].count()
    # Filter the data for gender 2 (female) and METS status 1 (developed METS)
    female_mets = data[(data['GENDER'] == 2) & (data['mets_M0'] == 1)]['id'].count()
    ...
    male_non_smoker = data[(data['GENDER'] == 1) & (data['FUM_M0'] == 0)].shape[0]
    # Filter the data for gender 1 (male) and smoking status 1 (smoker)
    male_smoker = data[(data['GENDER'] == 1) & (data['FUM_M0'] == 1)].shape[0]

    # Filter the data for gender 2 (female) and smoking status 0 (non-smoker)
    female_non_smoker = data[(data['GENDER'] == 2) & (data['FUM_M0'] == 0)].shape[0]
    # Filter the data for gender 2 (female) and smoking status 1 (smoker)
    female_smoker = data[(data['GENDER'] == 2) & (data['FUM_M0'] == 1)].shape[0]
    ...
    # Calculate the median weight values for each time point for gender 1 (male)
    weight_median_male = [data[data['GENDER'] == 1]['POIDS_M0'].median(),
                        data[data['GENDER'] == 1]['POIDS_M3'].median(),
                        data[data['GENDER'] == 1]['POIDS_M6'].median()]

    # Calculate the median weight values for each time point for gender 2 (female)
    weight_median_female = [data[data['GENDER'] == 2]['POIDS_M0'].median(),
                        data[data['GENDER'] == 2]['POIDS_M3'].median(),
                        data[data['GENDER'] == 2]['POIDS_M6'].median()]
